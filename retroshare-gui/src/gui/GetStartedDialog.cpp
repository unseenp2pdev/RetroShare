...
void GetStartedDialog::inviteFriends()
{
    ...
    sendMail("", tr("UnseenP2P Invitation"), text);
}

QString GetStartedDialog::GetInviteText()
{
    QString text = tr("Your friend has installed UnseenP2P, and would like you to try it out.") + "\n";
	text += "\n";
    text += tr("You can get UnseenP2P here: %1").arg(URL_DOWNLOAD) + "\n";
	text += "\n";
    text += tr("UnseenP2P is a private Friend-2-Friend sharing network.") + "\n";
    ...
    text += tr("Cut and paste the text below into your UnseenP2P client") + "\n";
...
void GetStartedDialog::emailSubscribe()
{
    ...
    QString text = QString("Please let me know when UnseenP2P has a new release, or exciting news") + "\n";
	text += "\n";
...
{
	// when translation is needed, replace QString by tr
    QString text = QString("I am no longer interested in UnseenP2P News.") + "\n";
...
{
	// when translation is needed, replace QString by tr
    QString text = QString("Dear UnseenP2P Developers") + "\n";
	text += "\n";
    text += QString("I've tried out UnseenP2P and would like provide feedback:") + "\n";
	text += "\n";
    text += QString("To make UnseenP2P more user friendly, please [ what do you think? ] ") + "\n";
    text += QString("The best feature of UnseenP2P is [ what do you think? ] ") + "\n";
	text += QString("and the biggest missing feature is [ what do you think? ] ") + "\n";
	text += "\n";
	text += QString("Furthermore, I'd like to say ... ") + "\n";
	text += "\n";

    sendMail("feedback@retroshare.org", tr("UnseenP2P Feedback"), text);
}
...
//			break;
  ...
  #endif
#endif
    text += QString("My UnseenP2P Configuration is: (%1, %2, 0x60%3)").arg(Rshare::retroshareVersion(true)).arg(sysVersion).arg(userLevel) + "\n";
    text += "\n";

    text += QString("I am having trouble with UnseenP2P.");
	text += QString(" Can you help me with....") + "\n";
	text += "\n";

    sendMail("support@retroshare.org", tr("UnseenP2P Support"), text);
}
