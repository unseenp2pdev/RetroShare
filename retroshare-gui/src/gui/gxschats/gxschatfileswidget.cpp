...
#include "GxsChatFilesWidget.h"
#include "ui_GxsChatFilesWidget.h"
#include "GxsChatFilesStatusWidget.h"
...
GxsChatFilesWidget::GxsChatFilesWidget(QWidget *parent) :
    QWidget(parent), ui(new Ui::GxsChatFilesWidget)
{
    ui->setupUi(this);
...
GxsChatFilesWidget::~GxsChatFilesWidget()
{
    delete(mCompareRole);
    delete ui;
}

void GxsChatFilesWidget::addFiles(const RsGxsChatMsg &post, bool related)
{
    if (related) {
    ...
    for (fileIt = post.mFiles.begin(); fileIt != post.mFiles.end(); ++fileIt) {
        ...
        QWidget *statusWidget = new GxsChatFilesStatusWidget(post.mMeta.mGroupId, post.mMeta.mMsgId, file);
...
void GxsChatFilesWidget::clear()
{
    ui->treeWidget->clear();
    closeFeedItem();
}

void GxsChatFilesWidget::setFilter(const QString &text, int type)
{
...
void GxsChatFilesWidget::setFilterText(const QString &text)
{
    setFilter(text, mFilterType);
}

void GxsChatFilesWidget::setFilterType(int type)
{
    setFilter(mFilterText, type);
}

void GxsChatFilesWidget::filterItems()
{
...
//QTreeWidgetItem *GxsChatFilesWidget::findFile(const RsGxsGroupId &groupId, const RsGxsMessageId &messageId, const RsFileHash &fileHash)
//{
//	QTreeWidgetItemIterator it(ui->treeWidget);
//	QTreeWidgetItem *treeItem;
//	while ((treeItem = *it) != NULL) {
...
void GxsChatFilesWidget::removeItems(const RsGxsGroupId &groupId, const RsGxsMessageId &messageId)
{
    QTreeWidgetItemIterator it(ui->treeWidget);
    QTreeWidgetItem *treeItem;
    while ((treeItem = *it) != NULL) {
...
void GxsChatFilesWidget::closeFeedItem()
{
    if (mFeedItem) {
        delete(mFeedItem);
        mFeedItem = NULL;
...
void GxsChatFilesWidget::currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem */*previous*/)
{
    if (!current) {
        closeFeedItem();
        return;
    ...
    mFeedItem = new GxsChatPostItem(NULL, 0, groupId, messageId, true, true);
