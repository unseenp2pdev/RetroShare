...
GxsChatWidget::GxsChatWidget(const RsGxsGroupId &groupId, QWidget *parent)
...
GxsChatWidget::~GxsChatWidget()
{
	processSettings(false);

	/* Cleanup plugin functions */
    foreach (GxsChatWidgetHolder *chatWidgetHolder, mChatWidgetHolder) {
		delete(chatWidgetHolder);
    }

	delete ui;
}
//gxs overload functions

void GxsChatWidget::clearPosts()
{
//    ui->feedWidget->clear();
//    ui->fileWidget->clear();
}

void GxsChatWidget::blank()
{
...
bool GxsChatWidget::navigatePostItem(const RsGxsMessageId &msgId)
{
...
void GxsChatWidget::subscribeGroup(bool subscribe)
{
    if (groupId().isNull()) {
        return;
    }
...
void GxsChatWidget::groupNameChanged(const QString &name)
{
//    if (groupId().isNull()) {
...
void GxsChatWidget::fillThreadCreatePost(const QVariant &post, bool related, int current, int count)
{
//    /* show fill progress */
//    if (count) {
...
void GxsChatWidget::setAllMessagesReadDo(bool read, uint32_t &token)
{
...
void GxsChatWidget::insertAllPosts(const uint32_t &token, GxsMessageFramePostThread *thread)
{
...
void GxsChatWidget::insertPosts(const uint32_t &token)
{
    std::vector<RsGxsChatMsg> posts;
    rsGxsChats->getPostData(token, posts);
...
bool GxsChatWidget::insertGroupData(const uint32_t &token, RsGroupMetaData &metaData)
{
...
void GxsChatWidget::insertChannelDetails(const RsGxsChatGroup &group)
{
    /* IMAGE */
    QPixmap chanImage;
    ...
    bool withUnreadCount=0;
    ui->titleLabel->setText(RsHtml::plainText(groupName(withUnreadCount)));
    init(chatId,groupName(withUnreadCount));

//    ui->logoLabel->setPixmap(chanImage);

//    ui->subscribersLabel->setText(QString::number(group.mMeta.mPop)) ;

    if (group.mMeta.mSubscribeFlags & GXS_SERV::GROUP_SUBSCRIBE_PUBLISH) //public channel, not yet subscribe
    {
...
void GxsChatWidget::setDefaultExtraFileFlags(TransferRequestFlags fl)
{
	mDefaultExtraFileFlags = fl ;
	ui->hashBox->setDefaultTransferRequestFlags(fl) ;
}

void GxsChatWidget::addChatHorizontalWidget(QWidget *w)
{
	ui->pluginsVLayout->addWidget(w) ;
	update() ;
}

void GxsChatWidget::addChatBarWidget(QWidget *w)
{
	int iconHeight = QFontMetricsF(font()).height();
	double fmm = iconHeight > FMM_THRESHOLD ? FMM : FMM_SMALLER;
	iconHeight *= fmm;
...
void GxsChatWidget::addTitleBarWidget(QWidget *w)
{
	ui->pluginTitleFrame->layout()->addWidget(w) ;
}

void GxsChatWidget::hideChatText(bool hidden)
{
	ui->chatTextFrame->setHidden(hidden); ;
}

RSButtonOnText* GxsChatWidget::getNewButtonOnTextBrowser()
{
	return new RSButtonOnText(ui->textBrowser);
}

RSButtonOnText* GxsChatWidget::getNewButtonOnTextBrowser(QString text)
{
	return new RSButtonOnText(text, ui->textBrowser);
}


void GxsChatWidget::init(const ChatId &chat_id, const QString &title)
{
    this->chatId = chat_id;
	this->title = title;
...
//		if (plugin) {
//            GxsChatWidgetHolder *chatWidgetHolder = plugin->qt_get_chat_widget_holder(this);
//			if (chatWidgetHolder) {
...
ChatId GxsChatWidget::getChatId()
{
    return chatId;
}

GxsChatWidget::ChatType GxsChatWidget::chatType()
{
...
void GxsChatWidget::blockSending(QString msg)
{
...
void GxsChatWidget::unblockSending()
{
    sendingBlocked = false;
    updateLenOfChatTextEdit();
}

void GxsChatWidget::processSettings(bool load)
{
...
uint32_t GxsChatWidget::maxMessageSize()
{
...
bool GxsChatWidget::eventFilter(QObject *obj, QEvent *event)
{
 ...
 */
void GxsChatWidget::completeNickname(bool reverse)
{
...
QAbstractItemModel *GxsChatWidget::modelFromPeers()
{
    // Find lobby we belong to
    ChatLobbyInfo lobby ;
...
void GxsChatWidget::addToolsAction(QAction *action)
{
	ui->pushtoolsButton->menu()->addAction(action);
}

void GxsChatWidget::showEvent(QShowEvent */*event*/)
{
	newMessages = false;
	emit infoChanged(this);
...
void GxsChatWidget::resizeEvent(QResizeEvent */*event*/)
{
	// Workaround: now the scroll position is correct calculated
	QScrollBar *scrollbar = ui->textBrowser->verticalScrollBar();
	scrollbar->setValue(scrollbar->maximum());
...
void GxsChatWidget::addToParent(QWidget *newParent)
{
	newParent->window()->installEventFilter(this);
}

void GxsChatWidget::removeFromParent(QWidget *oldParent)
{
	oldParent->window()->removeEventFilter(this);
}

void GxsChatWidget::focusDialog()
{
	ui->chatTextEdit->setFocus();
}

QToolButton* GxsChatWidget::getNotifyButton()
{
	if (ui) if (ui->notifyButton) return ui->notifyButton;
	return NULL;
}

void GxsChatWidget::setNotify(GxsChatUserNotify *clun)
{
	if(clun) notify=clun;
}

void GxsChatWidget::on_notifyButton_clicked()
{
	if(!notify) return;
...
void GxsChatWidget::scrollToAnchor(QString anchor)
{
	ui->textBrowser->scrollToAnchor(anchor);
}

void GxsChatWidget::setWelcomeMessage(QString &text)
{
	ui->textBrowser->setText(text);
}

void GxsChatWidget::addChatMsg(bool incoming, const QString &name, const QDateTime &sendTime, const QDateTime &recvTime, const QString &message, MsgType chatType)
{
	addChatMsg(incoming, name, RsGxsId(), sendTime, recvTime, message, chatType);
}

void GxsChatWidget::addChatMsg(bool incoming, const QString &name, const RsGxsId gxsId
...
void GxsChatWidget::pasteText(const QString& S)
{
...
void GxsChatWidget::contextMenuTextBrowser(QPoint point)
{
	QMatrix matrix;
...
void GxsChatWidget::contextMenuSearchButton(QPoint /*point*/)
{
...
void GxsChatWidget::chatCharFormatChanged()
{
...
void GxsChatWidget::resetStatusBar()
{
...
void GxsChatWidget::updateStatusTyping()
{
...
void GxsChatWidget::updateLenOfChatTextEdit()
{
...
void GxsChatWidget::on_closeInfoFrameButton_clicked()
{
	ui->infoFrame->setVisible(false);
}

void GxsChatWidget::on_searchButton_clicked(bool bValue)
{
	if (firstSearch)
...
void GxsChatWidget::on_searchBefore_clicked()
{
	findText(ui->leSearch->text(),true,true);
}
void GxsChatWidget::on_searchAfter_clicked()
{
	findText(ui->leSearch->text(),false,true);
}

void GxsChatWidget::toogle_FindCaseSensitively()
{
	bFindCaseSensitively=!bFindCaseSensitively;
}

void GxsChatWidget::toogle_FindWholeWords()
{
	bFindWholeWords=!bFindWholeWords;
}

void GxsChatWidget::toogle_MoveToCursor()
{
	bMoveToCursor=!bMoveToCursor;
}

void GxsChatWidget::toogle_SeachWithoutLimit()
{
	bSearchWithoutLimit=!bSearchWithoutLimit;
}

bool GxsChatWidget::findText(const QString& qsStringToFind)
{
	return findText(qsStringToFind, false,false);
}

bool GxsChatWidget::findText(const QString& qsStringToFind, bool bBackWard, bool bForceMove)
{
	QTextDocument *qtdDocument = ui->textBrowser->document();
	bool bFound = false;
...
void GxsChatWidget::removeFoundText()
{
...
void GxsChatWidget::on_markButton_clicked(bool bValue)
{
	if (bValue)
	{
		if (ui->textBrowser->textCursor().selectedText().length()>0)
...
void GxsChatWidget::chooseColor()
{
	bool ok;
...
void GxsChatWidget::colorChanged()
{
	QPixmap pix(16, 16);
...
void GxsChatWidget::resetFont()
{
	currentFont.fromString(Settings->getChatScreenFont());
	setFont();
}

void GxsChatWidget::resetFonts()
{
	currentFont.fromString(Settings->getChatScreenFont());
	setColorAndFont(true);
...
void GxsChatWidget::setColorAndFont(bool both)
{
...
void GxsChatWidget::setFont()
{
	setColorAndFont(false);
    PeerSettings->setPrivateChatFont(chatId, currentFont.toString());
}

void GxsChatWidget::smileyWidget()
{
	Emoticons::showSmileyWidget(this, ui->emoteiconButton, SLOT(addSmiley()), true);
}

void GxsChatWidget::addSmiley()
{
...
void GxsChatWidget::clearChatHistory()
{
...
void GxsChatWidget::deleteChatHistory()
{
...
void GxsChatWidget::messageHistory()
{
    ImHistoryBrowser imBrowser(chatId, ui->chatTextEdit, window());
	imBrowser.exec();
}

void GxsChatWidget::addExtraFile()
{
	QStringList files;
...
void GxsChatWidget::addExtraPicture()
{
...
void GxsChatWidget::fileHashingFinished(QList<HashedFile> hashedFiles)
{
...
void GxsChatWidget::setCurrentFileName(const QString &fileName)
{
...
void GxsChatWidget::updateStatus(const QString &peer_id, int status)
{
...
//	    }
        if (!groupId().isNull()) {
            bool withUnreadCount=0;
            ui->titleLabel->setText(groupName(withUnreadCount));
        }else
...
void GxsChatWidget::updateTitle()
{
	if (chatType() != CHATTYPE_LOBBY) {
...
void GxsChatWidget::updatePeersCustomStateString(const QString& /*peer_id*/, const QString& /*status_string*/)
{
	QString status_text;
...
void GxsChatWidget::updateStatusString(const QString &statusMask, const QString &statusString, bool permanent)
{
...
void GxsChatWidget::setName(const QString &name)
{
	this->name = name;
	updateTitle();
}

bool GxsChatWidget::setStyle()
{
...
void GxsChatWidget::setUseCMark(const bool bUseCMark)
{
	useCMark = bUseCMark;
...
void GxsChatWidget::updateCMPreview()
{
	if (!useCMark) return;
...
void GxsChatWidget::dropPlacemark()
{
