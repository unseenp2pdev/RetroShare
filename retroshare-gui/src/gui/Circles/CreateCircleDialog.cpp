...
void CreateCircleDialog::createCircle()
{
    ...
    if(name.isEmpty()) {
	    /* error message */
        QMessageBox::warning(this, tr("UnseenP2P"),tr("Please set a name for your Circle"), QMessageBox::Ok, QMessageBox::Ok);
    ...
    //		QMessageBox::warning(this, tr("UnseenP2P"),tr("Cannot Edit Existing Circles Yet"), QMessageBox::Ok, QMessageBox::Ok);
    //		return; 
    //	}
    ...
    if (mIsExternalCircle) 
    {
	    ...
	    if (ui.radioButton_Public->isChecked()) {
		    ...
		    if (ui.circleComboBox->getChosenCircle(chosenId)) {
                ...
                QMessageBox::warning(this, tr("UnseenP2P"),tr("No Restriction Circle Selected"), QMessageBox::Ok, QMessageBox::Ok);
			    return; 
		    }//else (ui.circleComboBox->getChosenCircle(chosenId))
	    } 
	    else 
	    {
            QMessageBox::warning(this, tr("UnseenP2P"),tr("No Circle Limitations Selected"), QMessageBox::Ok, QMessageBox::Ok);
		    return; 
	    }
    } 
    else 
