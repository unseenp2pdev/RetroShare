...
//	setPixmap(QWizard::WatermarkPixmap, QPixmap(":/images/connectFriendWatermark.png"));
    ...
    subject = tr("UnseenP2P Invitation");
...
void ConnectFriendWizard::initializePage(int id)
{
	...
	case Page_Email:
        ...
        ui->subjectEdit->setText(tr("UnseenP2P Invitation"));
	...
	if (cert.empty()) {
		...
		if (rsPeers->cleanCertificate(cert, cleanCert, error_code)) {
			...
			if (error_code > 0) {
				...
				default:
                    errorMsg = tr("Not a valid UnseenP2P certificate!") ;
...
{
	QString fileName ;
    if(!misc::getOpenFileName(this, RshareSettings::LASTDIR_CERT, tr("Select Certificate"), tr("UnseenP2P Certificate (*.rsc );;All Files (*)"),fileName))
		return ;
...
void ConnectFriendWizard::generateCertificateCalled()
{
	...
	if (cert.empty()) {
        QMessageBox::information(this, "UnseenP2P", tr("Sorry, create certificate failed"), QMessageBox::Ok, QMessageBox::Ok);
		return;
	}

    QString qdir = QFileDialog::getSaveFileName(this, tr("Please choose a filename"), QDir::homePath(), tr("UnseenP2P Certificate (*.rsc );;All Files (*)"));
	...
	if (qdir.isEmpty() == false) {
		...
		if (CertFile.open(QIODevice::WriteOnly/* | QIODevice::Text*/)) {
			if (CertFile.write(QByteArray(cert.c_str())) > 0) {
                QMessageBox::information(this, "UnseenP2P", tr("Certificate file successfully created"), QMessageBox::Ok, QMessageBox::Ok);
			} else {
                QMessageBox::information(this, "UnseenP2P", tr("Sorry, certificate file creation failed"), QMessageBox::Ok, QMessageBox::Ok);
			}
			CertFile.close();
		} else {
            QMessageBox::information(this, "UnseenP2P", tr("Sorry, certificate file creation failed"), QMessageBox::Ok, QMessageBox::Ok);
