...
void DistributedChatService::saveGroupChatInfo()
{
    triggerConfigSave();
}
...
bool DistributedChatService::acceptLobbyInvite(const ChatLobbyId& lobby_id,const RsGxsId& identity)
{
    ...
    //unseenp2p - try to save all memberlist of lobbychat (groupchat) info to local
    triggerConfigSave() ;
	return true ;
}
...
bool DistributedChatService::processLoadListItem(const RsItem *item)
{
    ...
    if(NULL != (clinfo = dynamic_cast<const RsChatLobbyInfoItem*>(item)))
    {
        ...
        entry.connexion_challenge_count = 0 ;
        entry.gxs_ids = clinfo->lobbyInfo.gxs_ids; //add more this
