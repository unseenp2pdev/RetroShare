...
void p3GxsChatService::sendPublicChat(const std::string &msg)
{
...
class p3GxsChatService::AvatarInfo
{
   public:
      AvatarInfo()
      {
...
void p3GxsChatService::sendGroupChatStatusString(const std::string& status_string)
{
    std::set<RsPeerId> ids;
    mServiceCtrl->getPeersConnected(getServiceInfo().mServiceType, ids);
...
void p3GxsChatService::sendStatusString( const ChatId& id,
...
void p3GxsChatService::clearChatLobby(const ChatId& id)
{
...
void p3GxsChatService::checkSizeAndSendMessage(RsChatMsgItem *msg)
{
...
bool p3GxsChatService::isOnline(const RsPeerId& pid)
{
...
bool p3GxsChatService::sendChat(ChatId destination, std::string msg)
{
...
bool p3GxsChatService::locked_checkAndRebuildPartialMessage(RsChatMsgItem *& ci)
{
...
void p3GxsChatService::receiveChatQueue()
{
    RsItem *item ;
...
void p3GxsChatService::handleRecvChatAvatarItem(RsChatAvatarItem *ca)
{
...
uint32_t p3GxsChatService::getMaxMessageSecuritySize(int type)
{
    switch (type)
    {
...
bool p3GxsChatService::checkForMessageSecurity(RsChatMsgItem *ci)
{
...
bool p3GxsChatService::initiateDistantChatConnexion( const RsGxsId& to_gxs_id,
...
bool p3GxsChatService::receiveGxsTransMail( const RsGxsId& authorId,
...
bool p3GxsChatService::notifyGxsTransSendStatus(RsGxsTransId mailId,
...
bool p3GxsChatService::handleRecvChatMsgItem(RsChatMsgItem *& ci)
{
    std::string name;
...
void p3GxsChatService::locked_storeIncomingMsg(RsChatMsgItem */*item*/)
{
#ifdef REMOVE
...
void p3GxsChatService::initChatMessage(RsChatMsgItem *c, ChatMessage &m)
{
...
void p3GxsChatService::setOwnCustomStateString(const std::string& s)
{
...
void p3GxsChatService::setOwnAvatarJpegData(const unsigned char *data,int size)
{
    {
        RsStackMutex stack(mChatMtx); /********** STACK LOCKED MTX ******/
#ifdef GXSCHAT_DEBUG
...
std::string p3GxsChatService::getOwnCustomStateString()
{
    RsStackMutex stack(mChatMtx); /********** STACK LOCKED MTX ******/
    return _custom_status_string ;
}
void p3GxsChatService::getOwnAvatarJpegData(unsigned char *& data,int& size)
{
...
std::string p3GxsChatService::getCustomStateString(const RsPeerId& peer_id)
{
    {
        // should be a Mutex here.
        RsStackMutex stack(mChatMtx); /********** STACK LOCKED MTX ******/
...
void p3GxsChatService::getAvatarJpegData(const RsPeerId& peer_id,unsigned char *& data,int& size)
{
    {
        // should be a Mutex here.
        RsStackMutex stack(mChatMtx); /********** STACK LOCKED MTX ******/
...
void p3GxsChatService::sendAvatarRequest(const RsPeerId& peer_id)
{
    if(!isOnline(peer_id))
        return ;
...
void p3GxsChatService::sendCustomStateRequest(const RsPeerId& peer_id){
...
RsChatStatusItem *p3GxsChatService::makeOwnCustomStateStringItem()
{
    RsStackMutex stack(mChatMtx); /********** STACK LOCKED MTX ******/
...
RsChatAvatarItem *p3GxsChatService::makeOwnAvatarItem()
{
    RsStackMutex stack(mChatMtx); /********** STACK LOCKED MTX ******/
...
bool p3GxsChatService::loadList(std::list<RsItem*>& load)
{
...
bool p3GxsChatService::saveList(bool& cleanup, std::list<RsItem*>& list)
{
    cleanup = true;
...
void p3GxsChatService::saveDone()
{
    /* unlock mutex */
    mChatMtx.unlock(); /****** MUTEX UNLOCKED *******/
}

RsSerialiser *p3GxsChatService::setupSerialiser()
{
...
void p3GxsChatService::statusChange(const std::list<pqiServicePeer> &plist)
{
