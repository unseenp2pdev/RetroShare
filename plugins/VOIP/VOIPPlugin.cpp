...
QDialog *VOIPPlugin::qt_about_page() const
{
	...
	if(about_dialog == NULL)
	{
		...
		QString text ;
        text += QObject::tr("<h3>UnseenP2P VOIP plugin</h3><br/>   * Contributors: Cyril Soler, Josselin Jacquard<br/>") ;
        text += QObject::tr("<br/>The VOIP plugin adds VOIP to the private chat window of UnseenP2P. To use it, proceed as follows:<UL>") ;
...
{
    return QApplication::translate("VOIP", "This plugin provides voice communication between friends in UnseenP2P.").toUtf8().constData();
}
